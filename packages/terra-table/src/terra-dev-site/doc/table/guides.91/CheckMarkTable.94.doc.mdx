...
```
Now pass the key from the rowData to our `getIsRowChecked` method.
```diff
  const createRow = rowData => (
    {
      key: rowData.key,
...
```
We also are going to implement disclosure on our rows, so we will need to set out `discloseLabel` and the `primaryCellIndex`. The `primaryCellIndex` relates to the cell that will function as an accessible link element in our row.
```diff
  const createRow = rowData => (
    {
      key: rowData.key,

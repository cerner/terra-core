...
module.exports = resizeTo(['tiny', 'huge'], {
  'Displays a default table with the provided text': (browser) => {
    browser.url(`${browser.launchUrl}/#/raw/tests/table/table/components/striped-table`);
  ...
  'Displays a table without the stripes': (browser) => {
    browser.url(`${browser.launchUrl}/#/raw/tests/table/table/components/no-striped-table`);
    browser.expect.element('#Table').to.be.present;
    browser.expect.element('[class*="striped"]').to.not.be.present;
  },

  'Displays a table without padding': (browser) => {
    browser.url(`${browser.launchUrl}/#/raw/tests/table/table/components/no-padding-table`);
    browser.expect.element('#Table').to.be.present;
  ...
  'Display a table with a sorting indicator': (browser) => {
    browser
      .url(`${browser.launchUrl}/#/raw/tests/table/table/components/table-with-sort-indicator`)
      .assert.attributeContains('th:nth-child(2)', 'data-sort', 'asc');
  },

  'Display a table with selected rows': (browser) => {
    browser
      .url(`${browser.launchUrl}/#/raw/tests/table/table/components/table-with-highlighted-rows`)
      .expect.element('tr[class*="is-selected"]').to.be.present;
  },

  'Display a table with a subheader': (browser) => {
    browser.url(`${browser.launchUrl}/#/raw/tests/table/table/components/table-with-subheaders`);
  ...
  'Displays a table with only one row': (browser) => {
    browser.url(`${browser.launchUrl}/#/raw/tests/table/table/components/single-row-table`);
  ...
  'Displays a table with no rows': (browser) => {
    browser.url(`${browser.launchUrl}/#/raw/tests/table/table/components/table-no-rows`);

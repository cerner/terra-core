...
it('should render a date picker with disabled dates', () => {
  const datePicker = <DatePicker excludeDates={[moment('04-01-2016', format), moment('04-03-2016', format)]} />;
  const wrapper = shallow(datePicker);
  expect(wrapper).toMatchSnapshot();
});
...
it('should render a date picker with included dates', () => {
  const datePicker = <DatePicker includeDates={[moment('04-01-2016', format), moment('04-02-2016', format), moment('04-03-2016', format)]} />;
  const wrapper = shallow(datePicker);
  expect(wrapper).toMatchSnapshot();
});

it('should render a date picker with min and max dates', () => {
  const datePicker = <DatePicker minDate={moment('04-01-2016', format)} maxDate={moment('04-10-2016', format)} />;
  const wrapper = shallow(datePicker);
  expect(wrapper).toMatchSnapshot();
});

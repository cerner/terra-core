...
class DateUtil {
  ...
  static filterInvalidDates(dates) {
    ...
    if (dates) {
      ...
      for (index = 0; index < dates.length; index += 1) {
        const momentDate = moment.utc(dates[index]);
  ...
  // Converts date string to the ISO8601 format in UTC. If the date string is invalid and unable to convert, the originally provided string is returned.
  static convertToISO8601(date, format) {
    if (date && format) {
      const momentDate = moment(date, format, true);
      return momentDate.isValid() ? momentDate.utc().format() : date;
    }

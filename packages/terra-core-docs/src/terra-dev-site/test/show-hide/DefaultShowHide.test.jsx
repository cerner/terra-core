...
class DefaultShowHide extends React.Component {
  ...
  render() {
    return (
      <ShowHide focusRef={this.focusRef} isOpen={this.state.isOpen} preview={<p>{previewText}</p>} onChange={this.toggleShowHide}>
        <p>
          { previewText }
          <span id="focusRef" ref={this.focusRef}>{ text }</span>
        </p>
      </ShowHide>
    );
  }

...
| --- | --- | --- |
| Hyperlink | Represents a basic HTML hyperlink (a hypertext anchor) labeled by its contents. This variant must have an `href` prop. | `import Hyperlink from terra-hyperlink';` |
| Hyperlink Button | Represents a basic HTML button that triggers hyperlink navigation via the required `onClick` event handler prop. | `import HyperlinkButton from 'terra-hyperlink/lib/HyperlinkButton';` |

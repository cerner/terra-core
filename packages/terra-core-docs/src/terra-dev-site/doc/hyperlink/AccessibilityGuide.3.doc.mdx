...
| Hyperlink Button | When applications may need to use a button element and add an `onClick` function to navigate the user | `import HyperlinkButton from 'terra-hyperlink/lib/HyperlinkButton';` |
...
- Ensure that action on a hyperlink occurs on the mouse-up OR key-up event. 

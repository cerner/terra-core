...
#### A single paragraph of text
  ...
  - A `Paragraph` component allows `ShowHide` component to set focus on the beginning of the hidden portion of the text upon opening. For that the text splitted in the preview and the rest of the text portions has to be passed as string props, and the `focusRef` should be forvarder to `Paragraph` component.
  ...
  const MyComponent = () => {
    ...
    ...
    // Split text in the preview and the rest of the text portions.
    ...
    return (
      // Pass a focusRef prop to the ShowHide component.
      // Pass a preview node as a prop to ShowHide component 
      <ShowHide focusRef={ focusRef } preview={ <p>{ previewText }</p> } ... >
        // Use <Paragraph /> component inside the <p /> tag.
        // Forward focusRef to <Paragraph /> component.
        // Pass preview and the rest of the text as string props. 
        <p>
...
#### Multiple children
  ...
  - Dublicate the preview nodes as `ShowHide` children.
  ...
  ```jsx
    ...
    const MyComponent = () => {
      ...
      return (
        // Pass a focusRef prop to the ShowHide component
        // Pass a preview node as a prop to ShowHide component 
        <ShowHide focusRef={focusRef} preview={preview} ... >
          // Dublicate the preview nodes as children here as well.
          <h3>{header}</h3>
          <ul>
            <li>{list[0]}</li>
            <li>{list[1]}</li>
            // This is where the preview portion ends. Add Paragraph element and pass text as a prop, forward focusRef.
            <li>
...
#### First hidden child is an interactive elements (link, button, etc)
  - At the top level of your component declare a `ref`.
  ...
  ```jsx
    ...
    const MyComponent = () => {
      ...
      const focusRef = useRef(null);
        ...
        ...
      // The hidden portion of ShowHide content starts with link. 
      const previewText = 'Patients are requesting greater affordability and efficiency in healthcare.';
      const href = 'https://www.cerner.com';

      return (
        // Pass a focusRef prop to the ShowHide component.
        // Pass a preview text node as a prop to ShowHide component 
        <ShowHide focusRef={ focusRef } preview={ <p>{ previewText }</p> } ... >
          // Forward focusRef to the link directly, there is no need to use <Paragraph /> component.
          <a ref={focusRef} href={href} ... />
        </ShowHide>
      );
    }
  ```
...
In order for the component to be accessible, it's important to follow the inplimentation guidance. Even though the focusRef prop is not required for backward compatibility, a failure to use it will result in lack of accessibility.
...
## Examples
<DefaultShowHide title="Default ShowHide - a Paragraph of Text" />
<CustomButtonTextShowHide title="Custom Button Text ShowHide - List of Items" />

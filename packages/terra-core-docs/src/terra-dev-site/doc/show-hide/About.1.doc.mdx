...
<Notice variant="important" ariaLevel="3">
  ...
  To make the Show/Hide component accessible, you must use the `focusRef` prop. This prop allows assistive technologies such as screen readers to put focus on the hidden text and announce it when it is disclosed. The `focusRef` is optional for backward compatibility but is required to comply with accessibility requirements.

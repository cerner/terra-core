...
The accessibility of a switch is imperative because a switch is a control users use to turn a setting on or off. If switches are not accessible, it can prevent users from understanding what state the switch is in, or worse, prevent the users from interacting with the control. By implementing switches in an accessible way, we are ensuring as many users as possible can use the switch regardless of how they may interact with it (e.g., with a mouse, touch, speech input, keyboard only, or other assistive technology).  

It is critical that a switch is written to be keyboard accessible. Additionally, a switch must be programmatically understood as a switch so that users of assistive technologies can understand what type of a component they are on and how to interact with it. Users expect a switch to turn on or off a setting and not to do anything unexpected. For a switch to be highly usable and easily understood, it is critical that all switches be uniquely labeled and descriptive of what action(on/off) will be taken when activated. For example:  

- **Blind screen reader users** do not necessarily interact with pages in a linear fashion and can use a switch out of context. They need to know what kind of a component they are on and how to interact with It; different components have different expectations. They need the switch's label to be descriptive of its purpose.
- **People using speech input software** require unique Switch labels to effectively activate the Switch. 
- **Keyboard only users** navigate the page using only a keyboard. They expect a switch to be keyboard accessible and able to be activated using specific keys. Additionally, visible keyboard focus is crucial for this user to see where they are on the page.

## Accessibility Considerations

### Code Considerations

- Ensure all switches receive keyboard focus and can be only interacted through the keyboard.
...
- Switch must have an accessible name which is descriptive of its purpose.
  ...
  <Switch
     isChecked={value}
     abelText="Show Patients List in Menu"
     onChange={setValue}
  />
  ```
...
- When implementing the switch do not override default theme colors of switch to ensure it  be easily perceived against its background color. If changing the default colors, the text color must meet a minimum contrast ratio against the background color. See [**WCAG 1.4.3 Contrast Minimum**](https://www.w3.org/WAI/WCAG22/Understanding/contrast-minimum.html) and [**WCAG 1.4.11 Non-text Content**](https://www.w3.org/WAI/WCAG21/Understanding/non-text-contrast.html) for more information.
- To prevent accessibility issues, never use color as the only way to convey meaning within the interface. Always provide both color and text or a shape together to represent the meaning.
- Ensure the engineers understand the logical reading order of the page.
- Never add unexpected behaviors to the switch that could cause a change of context on focus or on activation of the switch. Ensure that the switch always meet the user’s expectations.
...
- After implementing the switch, ensure all interactable elements receive keyboard focus in the appropriate reading order and display a visible keyboard focus.

<Whitespace newline={2} />

## Usability Expectations:

- Users expect a switch to have a visual and matching programmatic label that is descriptive of its purpose.
- Users expect to see or hear that the actionable element in focus is a switch.
- Users expect to see or hear the current state of the Switch. For example, selected/not selected, on/off, yes/no, and so on.
- If the switch state is changed then the users expect to be informed (audibly or visually) of the new state.
- Users expect the changing of switch states to result in instantaneous feedback. They do not expect to take further action to save the setting change.
...
- Keyboard users expect the switch to have a visible focus indication when the keyboard focus is placed upon the switch.
...
- [**1.1.1 Non-text Content**](https://www.w3.org/WAI/WCAG21/Understanding/non-text-content.html) - Supports 
   ...
   - Engineers must ensure all visual relationships between a switch and content that conveys meaning must also be programmatically implemented to ensure that the visual relationships can be understood by screen reader users.
   - Content creators must provide the meaningful switch text (the visual label) that communicates what users should expect when the switch is activated (on/off). Content creates should also provide any visual relationships of a switch with other content to ensure engineers can programmatically convey the visual relationships.
...
- [**2.4.7 Focus Visible**](https://www.w3.org/WAI/WCAG21/Understanding/focus-visible.html) - Supports
  ...
  - Content creators must inform engineers that whenever the switch receives focus, the focus must be visible and maintain existing Terra Keyboard Focus styles.
...
- [**2.5.3 Label in Name**](https://www.w3.org/WAI/WCAG21/Understanding/label-in-name.html) — Supports 
  ...
  - Content creators must provide the engineer with the switch text (the visual labels) which must be unique and describe the purpose of the switch so it can be understood by a user out of context.
...
- [**4.1.2 Name,Role,Value**](https://www.w3.org/WAI/WCAG21/Understanding/name-role-value.html) - Supports
  - Terra provides the ability for `Name`, `Role`, and `Value` attributes to be programmatically determined so that they can be read by screen readers.

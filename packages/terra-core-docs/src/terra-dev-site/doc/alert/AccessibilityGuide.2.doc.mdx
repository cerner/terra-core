...
The use of each variant of this component should be consistent in order to provide the user the best experience.
...
| Role | Purpose |
| --- | --- |
| Alert Role | See <Hyperlink href="https://w3c.github.io/aria/#alert" variant="external">Alert</Hyperlink>[<sup>[2]</sup>](/components/cerner-terra-core-docs/alert/accessibility-guide#linked-references) - "A type of live region with important, and usually time-sensitive, information." |
| Status Role | See <Hyperlink href="https://w3c.github.io/aria/#status" variant="external">Status</Hyperlink>[<sup>[2]</sup>](/components/cerner-terra-core-docs/alert/accessibility-guide#linked-references) - "A type of live region whose content is advisory information for the user but is not important enough to justify an alert, often but not necessarily presented as a status bar." |

#### Assistive technologies

In combination with the Alert Criticality, screen reader users should understand the importance of the notification when presented.

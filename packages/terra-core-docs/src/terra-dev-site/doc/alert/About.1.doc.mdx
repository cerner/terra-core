...
Alerts are expected to interrupt the flow only when notifications (alerts) are of high criticality.
Less critical notification banners should display a brief/important message that attracts the user's attention without interrupting.
...
within an element with `aria-live="polite"` and `aria-atomic="false"` (if screen reader should only read new content).

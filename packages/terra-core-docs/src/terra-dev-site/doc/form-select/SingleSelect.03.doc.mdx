...
Consumers are encouraged to prefer "Single Select Field" component over "Single Select" component (where ever possible) as the latter does not provide labels necessary to support accessibility.

To create an accessible Single Select component, follow the guidelines below:
1. Ensure that select Field can receive keyboard focus.
2. Ensure that Select Field is implemented correctly to programmatically associate all related content (visible label, helper text, error messaging) to the field.
3. Ensure the visible select Label matches any artificially applied programmatic label.

...
`terra-status` does not comply with accessibility guidelines and is no longer supported. If you are using `terra-status`, consider replacing it with `terra-icon` or `terra-image`.

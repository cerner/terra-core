...
If implemented improperly or without alternative content, icons will not be understood by everyone.
...
The announcement icon <span role="presentation">(<IconAnnouncement/>)</span> in this example is informative, because it means _the following text is an announcement_. However, that meaning is only conveyed if the user can see the icon. Setting the icons's `a11yLabel="Announcement"` conveys the same information to a screen reader user.
...
The screen reader user will hear something like: <samp> Group: Playback Controls. Link image Rewind. Link image Play. Link Fast image Forward.</samp>
...
The first star icon <span role="presentation">(<FilledStar/>)</span> is meaningful and conveys they rating to the screen reader user. The other icons <span role="presentation">(<FilledDecorativeStar/>, and <EmptyDecorativeStar/>)</span> are decorative so that the screen reader won't mention them. Mentioning the other icons in the group provides no benefit and serves only to confuse the user.

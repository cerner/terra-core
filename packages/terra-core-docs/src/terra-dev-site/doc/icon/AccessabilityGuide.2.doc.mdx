...
Icons that perform some operation, like an icon that is part of a hyperlink or a button, are said to be functional icons. Functional icons, like informative icons, **must have alternative text in code** to ensure that assistive technologies pass the icon's meaning to the user.
...
The factory icon (<IconManufacturer/>) above is purely decorative. It provides an extra visual queue but is not necessary for the user to understand that the text, _2800 Rock Creek Pkwy…_, is the address of the manufacturer.

Since the icon is decorative, its code must set `title=""` to ensure screen readers do not announce the icon's presence. If the title is left out, then the user will hear "Image Manufactured by …". If the title is set to `title="Manufactured by"` like in the Informative example above, then the user would hear "Manufactured by Manufactured by…". Neither of these are correct. The correct choice is to set `title=""` so that the user hears:

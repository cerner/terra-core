...
import AnchorButtonAccess from "./example/AnchorButtonAccess";
import A11yDecorativeButton from "./example/A11yDecorativeButton";
...
- Always to use the button variant over the anchor-button variant when possible.
...
Ensure any non-uniquely labeld buttons have a unique programmatic name that communicates the button's purpose by incorporating visually related content into the ARIA-LABEL. See the Allegies "Add" button example above. Be sure the visible text of the button is the first word of the ARIA-LABEL.

##### Accessibility Guidance: When to use Anchor Button

![IMPORTANT](https://badgen.net/badge/UX/Accessibility/blue) The best accessibility practice is always to use the button variant if possible. Only use the anchor-button variant when the intended action cannot be implemented using the button variant. 

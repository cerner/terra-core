...
- Always use the button variant over the anchor-button variant when possible.
...
Ensure any non-uniquely labelded buttons have a unique programmatic name that communicates the button's purpose by incorporating visually related content into the ARIA-LABEL. See the Allergies "Add" button example above. Be sure the visible text of the button is the first word of the ARIA-LABEL.

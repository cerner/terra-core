...
Buttons are one of HTML's foundational elements, so making buttons. Buttons are a vital tool for creating an intuitive user experience. However, because buttons are so fundamental to the functionality of web content, inaccessible or improperly implemented Buttons can be a severe barrier to overall accessibility. 

It is critical that buttons are coded correctly to ensure they are keyboard accessible and properly convey that they are a button which performs an action. Additionally, all users expect buttons to be unique and descriptive of what action will be taken when activated are critical to ensure all users can effectively understand and use them. For example: 

- **Blind screen reader users** do not necessarily interact with pages in a linear fashion and can use a button out of context. They need the words within the button to be descriptive of purpose. 
...
The screen reader user will hear something like: <samp> Click Me: Button </samp>

##### Accessibility Guidance: Icon Buttons

For buttons that use icons that convey a meaning, it is critical to include alternate content that represents that icon meaning. [insert code example here]	 
...
Ensure any non-unique button label that is visually related to content in the page must be programmatically associated with its content. For example, a “learn more” button next to the word “allergies” must be appropriate associated with the word allergies. [insert code example here]
...
- Must provide engineers with a meaningful and descriptive button label that conveys the button's purpose. The best practice is to use a verb that describes the button action.
  - Avoid non-unique button labels unless the buttons perform the exact same function. 
...
Users expect Buttons to open a new page, take them somewhere on the page, or to download a file.
...
- [**1.1.1 Non-text Content**](https://www.w3.org/WAI/WCAG21/Understanding/non-text-content.html) – Partially Supports 
  ...
  - Engineers must ensure when the Icon conveys meaning, to add appropriate ALT text to create a meaningful Button label.  
    - And any non-meaningful/decorative Icon should be marked as decorative.  
      E.g.: When a button has a print icon with the word "instructions" then the Icon is conveying information thus must have alternate text 
      E.g.: When a button has a print icon with the word "instructions" then the Icon is conveying information thus must have alternate text 
    - When a button has the word "print" and the print Icon; Icon would not have an alternate text. 
  - Content creators must provide engineers with meaningful text when using meaningful icons. Or inform engineers when the icon is "decorative" only.  

- [**1.3.1 Info and Relationships**](https://www.w3.org/WAI/WCAG21/Understanding/info-and-relationships.html) — Partially Supports — Terra Button does allow users to set a programmatic label of the button (a text string) but does not allow consuming teams the ability to set extra programmatic context for buttons to ensure they are programmatically associated to other related content. 
  - When applicable, engineers must ensure that any visual relationship to the button is programmatically codify. E.g.: to non-unique button "learn more", must be programmatically associated to the words to describe the "learn more" button.  
  - Content creators, when applicable, if other text in the page describe a button purpose; make sure engineers understand and apply a programmatic association to the text.  
    If using formatting to convey information, it should be available in text. Any visual formatting must be available in text programmatically. Use words in addition to the format to convey necessary information.  

- [**2.1.1 Keyboard**](https://www.w3.org/WAI/WCAG21/Understanding/keyboard.html) — Supports 
  - Interactable Terra Buttons natively receive focus, be interacted with, and able to be acted upon using the keyboard. Consuming teams must ensure to not overwrite native behavior.  
    Engineers must ensure behavior is maintained.  
...
- [**1.4.1 Use of Color**](https://www.w3.org/WAI/WCAG21/Understanding/use-of-color.html) - Partially Supports 
  ...
  - Content Creators must ensure color is NOT the only method used to convey. Instead, use both color and text. 
...
- [**3.2.4 Consistent Identification**](https://www.w3.org/WAI/WCAG21/Understanding/consistent-identification) : Components that have the same functionality within a set of Web pages are identified consistently — Partially Supports

...
module.exports = resizeTo(['medium'], {
  'Displays a default list item': (browser) => {
    browser
      .url(`${browser.launchUrl}/#/raw/tests/terra-list/list/list-item/default-list-item`)
      .assert.elementPresent('li');
  },

  'Displays a selected list item': (browser) => {
    browser.url(`${browser.launchUrl}/#/raw/tests/terra-list/list/list-item/selected-list-item`);
    browser.expect.element('li').to.have.attribute('class').which.contains('selected');
  },

  'Displays a selectable list item': (browser) => {
    browser.url(`${browser.launchUrl}/#/raw/tests/terra-list/list/list-item/selectable-list-item`);
    browser.expect.element('li').to.have.attribute('class').which.contains('is-selectable');
  },

  'Displays a chevron list item': (browser) => {
    browser.url(`${browser.launchUrl}/#/raw/tests/terra-list/list/list-item/chevron-list-item`);

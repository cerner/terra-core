...
import { withKnobs, color } from '@kadira/storybook-addon-knobs';
...
storiesOf(' Status with ', module)
  .add(' image', () => {
    const value = color(label, defaultValue);
    return <Status color={value} > {image} </Status>;
  })
  .add(' icon', () => {
    const value = color(label, defaultValue);
    return <Status color={value} > {icon} </Status>;
  })
  .add(' text', () => {
    const value = color(label, defaultValue);
    return <Status color={value} > {simpleText} </Status>;
  })
  .add(' arrange', () => {
    const value = color(label, defaultValue);
    const content = (<Arrange fitStart={icon} fill={simpleText} fitEnd={image} alignFill="center" />);
    return <Status color={value} > {content} </Status>;
  });
